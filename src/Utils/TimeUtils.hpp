...
public:
	...
	static void DoWhileInGame(TInterval interval, TCallback&& callback, TDuration duration, const bool pausable = true, const bool timeScaled = true)
	{
		...
		TimeUtils::DoWhile(
			[getProgress, computeInterval, computeTimescale, durationSeconds]() {
				...
				if (durationSeconds >= std::chrono::duration<float>(std::chrono::steady_clock::duration::max()).count() / 2.0f) {
					return std::max(scaledInterval, FRAME_DELAY());
				}

				const float remainingFraction = 1.0f - getProgress();
				const auto  remainingNanos = std::chrono::nanoseconds(
                    static_cast<int64_t>(remainingFraction * durationSeconds * 1'000'000'000LL)
				);
	...
	static void DoWhile(TInterval interval, TCallback&& callback, const bool pausable = true)
	{
		...
		std::jthread([=, callback = std::forward<TCallback>(callback)]() mutable {
			...
			while (IsLooping(*result)) {
				...
				const auto lastTickTime = std::chrono::steady_clock::now();
				const auto nextTickTime = lastTickTime + currentInterval;
				...
				std::this_thread::sleep_for(currentInterval > FRAME_DELAY() ? currentInterval : FRAME_DELAY());
			}
